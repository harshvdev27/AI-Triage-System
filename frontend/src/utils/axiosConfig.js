/**
 * Axios Configuration with 250ms Timeout
...
const REQUEST_TIMEOUT = 250; // 250ms hard timeout
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
      console.error(`❌ Request timeout: ${error.config.url} exceeded 250ms`);
      ...
      return Promise.reject({
        message: 'Unable to complete request within 250ms. Please try again.',
        timeout: true
      });
    }
